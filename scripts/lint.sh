...
function solFormat() {
    ...
    forge fmt --root $REPO_PATH/contracts $REPO_PATH/contracts/src/**
}

function solFormatCheck() {
    # format solidity contracts
    echo "Checking formatting of Solidity contracts..."
    forge fmt --check --root $REPO_PATH/contracts $REPO_PATH/contracts/src/**
}
...
function golangLinter() {
    ...
    cd $REPO_PATH
    golangci-lint run --config=$REPO_PATH/.golangci.yml ./...
}
